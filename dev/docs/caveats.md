...
- **Grabs.** The pull never dispatches through a grab that supplies its own focus and does work in `motion` (every driftwm grab, smithay's click and DnD grabs): real input and `warp_pointer`'s grabbed branch drive those, and the first pull after the release repicks — because the grabbed dispatch blanked the record, not because the target changed. smithay's `PopupPointerGrab` is the exception: it forwards `under` to the popup's client and `None` to anyone else and does nothing else while live, so a menu keeps hover state as things move under a stationary cursor; `dispatch_pointer_motion` records what the grab forwards so the dedup still holds, and a grab that has ended is torn down through `tear_down_popup_grab` rather than dispatched into.
