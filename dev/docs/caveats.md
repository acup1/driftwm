...
- **Transients.** A fullscreen entry, an exit, a fit or a fill moves a window before the client commits at the new size, so for a few frames its committed rect is the old size at the new position. The pull holds while the *focused* window is in that state (`FullscreenEntry::awaiting_size`, or an owed `pending_recenter`) and the cursor is inside the rect it is heading for; otherwise a fullscreen game's lock would be dropped and re-armed with an absolute motion in between. A transitioning window under the cursor is picked as is — its origin is final.
