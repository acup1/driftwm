...
- **Transients.** A fullscreen entry, or an exit from fullscreen, fit or fill, moves a window before the client commits at the new size, so for a few frames its committed rect is the old size at the new position. The pull holds while the *focused* window is in that state (`FullscreenEntry::awaiting_size`, or an owed `pending_recenter`) and the cursor is inside the rect it is heading for; otherwise a fullscreen game's lock would be dropped and re-armed with an absolute motion in between. A transitioning window under the cursor is picked as is — its origin is final. The entry hold ends when the client answers with a commit at any size, the exit hold when the owed recentre is taken, which needs a commit at a size other than the pre-exit one; a client that never answers keeps its focus while the cursor stays inside the rect, which is the push world's behaviour for that one window.
- **Locks.** A locked constraint whose target is unchanged is never re-seated even when the origin moved; a confine is, because that cursor really moves. `update_pointer_constraint` runs on every pull that reaches the pick, dispatch or not, and on the transition hold: a persistent confine whose region is re-set around a parked cursor becomes armable with no delivery change and nothing else re-evaluates it.
