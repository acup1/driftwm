...
Every stage read in the input path takes the destination — `topmost_under`, `element_under_skipping`, `decoration_under` and `surface_under` all walk `Stage::entries`, whose `position` is the settled one (`src/input/mod.rs`) — while the render path draws at `geometry_visual_rect`, which has no consumer anywhere under `src/input/`. The two rects stay disjoint for the whole animation (for a fit, by `old_size/2 - usable.size/2 + outer_gap (+bar)` — hundreds of pixels), so a window can't be clicked where it is drawn: ~120 ms at default speeds, reading as a missed click, and ~4 s at `[effects] animation_speed = 0.02`. This looks like a bug and gets filed as one. It isn't.

The model: a window animation is eye candy over a state change that is already complete. The window is at its destination the moment the action runs and the picture is catching up, so hit-testing the destination is correct, and so are the surface-local coordinates `surface_under` derives from it. A *camera* animation is the opposite — the viewport genuinely is moving, so input tracks it live, which is why the grab-install chokepoints (`arm_interactive_move`, `begin_client_resize`) take the viewport out of flight rather than freezing input. Going visual-aware would mean threading `geometry_visual_rect(id).loc` into those four reads (checking `geometry_space` first — that accessor's doc warns about Canvas vs Screen), and it carries a non-obvious tax: the per-iteration pointer-focus pull (below) would have to hold off every animating window, not only the ones whose committed size is in flight, or moving pixels drag enter/leave across a stationary cursor. Do not reopen without revisiting the model first.

## Pointer focus is pulled once per iteration, never pushed

No site re-seats pointer focus because it changed the scene. `DriftWm::refresh_pointer_focus` (`src/input/mod.rs`) runs once per event-loop iteration from `refresh_and_flush_clients` — after `retain_alive`, popup cleanup and the deferred-adoption sweep, before the flush — and once more per rendered frame right after the animation tick in both render loops, so a camera warp's carried motion goes into the frame it was composed for. It re-picks with the same cascade real motion uses and dispatches only when the delivery — target, surface origin and surface-local point — differs from the record `dispatch_pointer_motion` keeps, so a recompute that would tell the client nothing costs no wire traffic. Nothing tracks `geometry().loc` on commit and there is no resync flag: the pull is the only mechanism. A handler about to act on focus may still call it to make focus current at that boundary — `on_pointer_button` and `on_pointer_axis` do, since a scene change and the press can share one dispatch, and so does `new_constraint` — which is a consumer, not a push.

Why a naive pull and not a dirty flag: the hit test reads state with no compositor chokepoint — input regions, subsurface trees and `set_geometry` land on any commit, popup positioners and layer arrangement on theirs, pick mode flips with the zoom. A flag set at every commit is the naive pull with more code, and a missed chokepoint is exactly the silent stale coordinate this replaced. Cost, measured in the fixture (`pull_cost_bench`, 20 windows, two layer surfaces, one pin, measured at 10 000 iterations on a 24-core box): a pull over bare canvas — the expensive path, every cascade arm — is 5.5 µs in release (34 µs debug), over a window 2.9 µs (22 µs), against 8.1 µs (60 µs) for one real absolute motion, which runs the same cascade plus hover, hot corners and edge pan. A 1 kHz mouse therefore pays well under one extra millisecond per second; on nested winit an idle session pays two pulls per 16 ms tick.

Three rules keep it honest, each with a test:

- **Grabs.** The pull never dispatches through a grab that supplies its own focus and does work in `motion` (every driftwm grab, smithay's click and DnD grabs): real input and `warp_pointer`'s grabbed branch drive those. driftwm's own grabs blank the record, so the first pull after their release repicks — because of the record, not because the target changed. smithay's click grab is recorded honestly instead (it keeps delivering to the surface the press landed on, at `under`'s origin while the cursor is still over it), so an in-client drag that ends where it began leaves nothing for the pull to send. smithay's `PopupPointerGrab` is the exception: it forwards `under` to the popup's client and `None` to anyone else and does nothing else while live, so a menu keeps hover state as things move under a stationary cursor; `dispatch_pointer_motion` records what the grab forwards so the dedup still holds, and the raw `under` beside it, because smithay restores focus from the last raw `under` when a grab ends — a foreign surface the grab filtered to `None` that then moved away would otherwise be re-entered for one flush. A grab that has ended is torn down through `tear_down_popup_grab` first — smithay's own teardown would rewrite keyboard focus from inside the pointer dispatch — and then dispatched into as no grab: the dead popup grab unsets itself and restores focus from the pick it is handed, so the restore is never one scene change behind.
- **Transients.** A fullscreen entry, or an exit from fullscreen, fit or fill, moves a window before the client commits at the new size, so for a few frames its committed rect is the old size at the new position. The pull holds while the *focused* window is in that state (`FullscreenEntry::awaiting_size`, or an owed `pending_recenter`) and the cursor is inside the rect it is heading for; otherwise a fullscreen game's lock would be dropped and re-armed with an absolute motion in between. The hold covers the delivery only: the hit test always runs, because it is what keeps `pointer_over_layer` current and `on_pointer_button` and `on_pointer_axis` read that flag before focus or the grab — held stale, a press or scroll over a bar the exit just revealed goes to the canvas, and the press path's own correcting hit test sits inside the branch the stale flag skips. A bar the exit uncovered is a real target that owes its enter, so `pointer_over_screen_space` releases the hold: it is for the window's own rect lagging its move, where the pick finds bare canvas, not for a layer that genuinely sits on top. A transitioning window under the cursor is picked as is — its origin is final. The entry hold ends when the client answers with a commit at any size, the exit hold when the owed recentre is taken, which needs a commit at a size other than the pre-exit one; a client that never answers keeps its focus while the cursor stays inside the rect.
- **Locks.** A locked constraint whose target is unchanged is never re-seated even when the origin moved; a confine is, because that cursor really moves. `update_pointer_constraint` runs on the transition hold and on every pull that reaches the delivery stage, dispatch or not — the redundant path included, because a persistent confine whose region is re-set around a parked cursor becomes armable with no delivery change and nothing else re-evaluates it. The one path that skips it is a locked constraint whose delivery is unchanged, which has nothing to re-evaluate.
