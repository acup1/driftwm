...
- **`with_pointer_constraint(surface, pointer, |c: Option<PointerConstraintRef<'_, D>>| ...) -> T`** hands the closure `None` both when the surface carries no constraint data at all and when it has data but no entry for this pointer. `PointerConstraintRef<'_, D>` derefs to `PointerConstraint`, an `enum { Confined(ConfinedPointer), Locked(LockedPointer) }` with `is_active(&self) -> bool` and `region(&self) -> Option<&RegionAttributes>`. `activate(&self)` and `deactivate(self)` are both idempotent — an `AtomicBool::swap` sends `locked`/`confined` or `unlocked`/`unconfined` only on the edge, and a real deactivation additionally does `remove_entry` when the constraint's lifetime is `Oneshot`. `RegionAttributes { pub rects: Vec<(RectangleKind, Rectangle<i32, Logical>)> }` is `Clone + Debug + Default` but not `PartialEq`, and offers `contains(point)`. driftwm reaches all of it through `src/input/constraint.rs`.
...
- **`TouchHandle::frame` and `cancel` (rev `4cf0b620`) warn and return when nothing reached `TouchInternal` since the last frame** (`frame called without prior events`). Only what a grab forwards through `TouchInnerHandle` counts, so a compositor grab that swallows a whole frame — a pan, a held-back finger — leaves nothing pending and the libinput frame that follows would log at warn level on every gesture frame. driftwm gates every `touch.frame` on `TouchState::frame_owed`, set at the delivery point (`TouchTarget for FocusTarget`). `cancel` is not gated: a bare cancel (palm rejection, a lock with fingers down) logs once and revokes nothing, which is also what the per-slot marker check does for a settled slot.
